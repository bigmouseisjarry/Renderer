...
static const char* DEVICE_EXTENTIONS[] = {
	...
	//VK_KHR_MULTIVIEW_EXTENSION_NAME,                  // 已经在vk1.3中改为VkPhysicalDeviceVulkan13Features指示，并入核心
};
...
public:
	...
	static uint32_t MultiviewCountToBitmask(uint32_t multiviewCount)
	{
		uint32_t bitmask = 0;
		for (uint32_t i = 0; i < multiviewCount; ++i) {
			bitmask |= (1 << i);
		}
		return bitmask;
	}
    ...
    static VkRenderingFlags RenderingFlagsToVk(RenderingFlags flags)
    {
        VkRenderingFlags vkFlags = 0;
        if (flags & RENDERING_CONTENTS_SECONDARY_COMMAND_BUFFERS) vkFlags |= VK_RENDERING_CONTENTS_SECONDARY_COMMAND_BUFFERS_BIT;
        if (flags & RENDERING_SUSPENDING) vkFlags |= VK_RENDERING_SUSPENDING_BIT;
        if (flags & RENDERING_RESUMING) vkFlags |= VK_RENDERING_RESUMING_BIT;
        return vkFlags;
    }
	...
	static VkResolveModeFlagBits ResolveModeToVk(ResolveMode resolveMode)
	{
		VkResolveModeFlagBits mode;
		switch (resolveMode) {
		case ResolveMode::RESOLVE_MODE_SAMPLE_ZERO:          mode = VK_RESOLVE_MODE_SAMPLE_ZERO_BIT;       break;
		case ResolveMode::RESOLVE_MODE_AVERAGE:              mode = VK_RESOLVE_MODE_AVERAGE_BIT;           break;
		case ResolveMode::RESOLVE_MODE_MIN:                  mode = VK_RESOLVE_MODE_MIN_BIT;               break;
		case ResolveMode::RESOLVE_MODE_MAX:                  mode = VK_RESOLVE_MODE_MAX_BIT;               break;
		default:                                LOG_FATAL("Unsupported resolve mode!");
		}
		return mode;
	}
