	...
	: RHIGraphicsPipeline(info)
{
    ...
    VkPipelineRenderingCreateInfo attachmentInfo = GetPipelineAttachmentsCreateInfo(colorAttachmentFormats, info.colorAttachmentFormats, info.depthStencilAttachmentFormat);
    ...
    pipelineInfo.pStages = shaderStages.data();
    pipelineInfo.layout = pipelineLayout;
    pipelineInfo.pNext = &attachmentInfo;
...
VkPipelineRenderingCreateInfo VulkanRHIGraphicsPipeline::GetPipelineAttachmentsCreateInfo(std::vector<VkFormat>& colorAttachmentFormatsVK, const std::array<RHIFormat, MAX_RENDER_TARGETS>& colorAttachmentFormats, RHIFormat depthStencilAttachmentFormat)
{
    VkPipelineRenderingCreateInfo attachmentInfo = {};
    attachmentInfo.sType = VK_STRUCTURE_TYPE_PIPELINE_RENDERING_CREATE_INFO;

    for (const RHIFormat& format : colorAttachmentFormats)
    {
        if (format == FORMAT_UKNOWN) break;
        colorAttachmentFormatsVK.push_back(VulkanUtil::RHIFormatToVkFormat(format));
    }

    attachmentInfo.colorAttachmentCount = (uint32_t)colorAttachmentFormatsVK.size();
    attachmentInfo.pColorAttachmentFormats = colorAttachmentFormatsVK.data();

    if (depthStencilAttachmentFormat != FORMAT_UKNOWN)
    {
        VkFormat vkFormat = VulkanUtil::RHIFormatToVkFormat(depthStencilAttachmentFormat);
        attachmentInfo.depthAttachmentFormat = vkFormat;
        attachmentInfo.stencilAttachmentFormat = IsStencilFormat(depthStencilAttachmentFormat) ? vkFormat : VK_FORMAT_UNDEFINED;
    }

    return attachmentInfo;
}

//VkPipelineRenderingCreateInfo VulkanRHIGraphicsPipeline::GetPipelineAttachmentsCreateInfo(const std::array<RHIFormat,MAX_RENDER_TARGETS>& colorAttachmentFormats, RHIFormat depthStencilAttachmentFormat)
//{
//    VkPipelineRenderingCreateInfo attachmentInfo = {};
//    attachmentInfo.sType = VK_STRUCTURE_TYPE_PIPELINE_RENDERING_CREATE_INFO;
//
//	std::vector<VkFormat> colorAttachmentFormatsVK;
//	for (const RHIFormat& format : colorAttachmentFormats)
//	{
//		if (format == FORMAT_UKNOWN) break;
//		colorAttachmentFormatsVK.push_back(VulkanUtil::RHIFormatToVkFormat(format));
...
//	if (depthStencilAttachmentFormat != FORMAT_UKNOWN)
//	{
//		attachmentInfo.depthAttachmentFormat = VulkanUtil::RHIFormatToVkFormat(depthStencilAttachmentFormat);
//		attachmentInfo.stencilAttachmentFormat = VulkanUtil::RHIFormatToVkFormat(depthStencilAttachmentFormat);
//	}
//
//	return attachmentInfo;
//}
