...
struct CrossQueueSyncPoint
{
    ...
    RDGPassNodeRef producer_pass;          // 生产者Pass
...
{
    // 原始同步需求（所有资源冲突点）
    std::vector<CrossQueueSyncPoint> raw_sync_points;

    // 优化后的同步点集合（SSIS算法结果）
    std::vector<CrossQueueSyncPoint> optimized_sync_points;
...
// 跨队列同步分析阶段 - 实现SSIS算法
class CrossQueueSyncAnalysis : public IRenderGraphPhase
{
public:
    ...
    // 调试接口
    void dump_ssis_analysis() const ;
    void dump_sync_points() const ;

private:
    void apply_ssis_optimization(RDGDependencyGraphRef graph) ;
    void calculate_optimization_statistics() ;

    // SSIS算法辅助结构
    struct SyncCoverage
    {
        RDGPassNodeRef node_to_sync_with;
        uint32_t node_index;
        std::vector<uint32_t> synced_queue_indices;
    };
...
private:
    ...
    std::unordered_map<RDGPassNodeRef, std::vector<uint32_t>> pass_ssis_;  // 每个Pass的SSIS数组
    std::unordered_map<RDGPassNodeRef, uint32_t> pass_local_to_queue_indices_; // Pass在队列内的本地执行索引
    std::unordered_map<RDGPassNodeRef, std::vector<RDGPassNodeRef>> pass_nodes_to_sync_with_; // 每个Pass需要同步的节点（会被优化）
